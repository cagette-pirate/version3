...
let currentPage = 1;
let itemsPerPage = 20;
let activeCategory = "all"; // Modifié de "alaune" à "all" pour afficher tout par défaut
...
let displayTimeout; // Variable pour gérer le debounce de l'affichage
...
function displayAnnonces(append = false) {
  ...
  // Annuler l'affichage précédent s'il est en attente pour éviter les doublons
  if (displayTimeout) {
...
document.querySelectorAll('.tag-filter').forEach(button => {
  button.addEventListener('click', () => {
    // Si le bouton cliqué est déjà actif, on le désactive et on remet "all"
    if (button.classList.contains('active')) {
      button.classList.remove('active');
      activeCategory = "all";
    } else {
      // Sinon, on active ce bouton et on désactive les autres
